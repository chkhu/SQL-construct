...
TEST_F(ExecutorTest, SimpleSeqScanTest) {
    ...
    auto out_schema = MakeOutputSchema({{"id",   col_a},
                                        {"name", col_b}});
...
TEST_F(ExecutorTest, SimpleDeleteTest) {
    ...
    auto r3 =
            GetExecutorContext()->GetCatalog()->CreateIndex("table-1", "index-1", index_keys, GetTxn(), index_info,
                                                            "bptree");
    ASSERT_EQ(DB_SUCCESS, r3);
    std::vector<IndexInfo *> indexes{index_info};
    std::vector<Row> result_set;
    auto scan_plan = std::make_shared<IndexScanPlanNode>(out_schema, table_info->GetTableName(), indexes, true,
                                                         predicate);

...
public:
    ...
    MakeColumnValueExpression(const Schema &schema, uint32_t row_idx, const std::string &col_name) {
        uint32_t col_idx;
    ...
    Schema *MakeOutputSchema(const std::vector<std::pair<std::string, AbstractExpressionRef>> &exprs) {
        ...
        for (const auto &input: exprs) {
            ...
            if (input.second->GetReturnType() != TypeId::kTypeChar) {
                ...
                cols.emplace_back(
                        new Column(input.first, input.second->GetReturnType(), MAX_VARCHAR_SIZE, col_idx, false,
                                   false));
            }
        }

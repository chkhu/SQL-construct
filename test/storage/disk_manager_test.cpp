...
TEST(DiskManagerTest, BitMapPageTest) {
  ...
  for(uint32_t i = 157 ; i < num_pages; i++){
...
TEST(DiskManagerTest, FreePageAllocationTest) {
  ...
  for(int i = 0; i < 5; i++) disk_mgr->AllocatePage();
  for (uint32_t i = 0; i < DiskManager::BITMAP_SIZE * extent_nums; i++) {
    DiskFileMetaPage *new_meta_page = reinterpret_cast<DiskFileMetaPage *>(disk_mgr->GetMetaData());
    if(i % 2 == 0){
      ASSERT_FALSE(disk_mgr->IsPageFree(i));
      disk_mgr->DeAllocatePage((i));
      ASSERT_TRUE(disk_mgr->IsPageFree(i));
      num_allocated_pages--;
      EXPECT_EQ(num_allocated_pages, new_meta_page->GetAllocatedPages());
      EXPECT_EQ(DiskManager::BITMAP_SIZE - (i%DiskManager::BITMAP_SIZE)/2-1, new_meta_page->GetExtentUsedPage(i/DiskManager::BITMAP_SIZE));
    }
  }

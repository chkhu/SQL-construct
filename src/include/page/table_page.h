...
class TablePage : public Page {
 public:
  ...
  uint8_t UpdateTuple(const Row &new_row, Row *old_row, Schema *schema, Transaction *txn, LockManager *lock_manager,
                      LogManager *log_manager);

  void ApplyDelete(const RowId &rid, Transaction *txn, LogManager *log_manager);

  void ApplyDeleteAll(Transaction *txn, LogManager *log_manager);
 ...
 private:
  ...
  static constexpr size_t INVALID_PARAMETER = 1;
  static constexpr size_t OUT_OF_MEMORY = 2;

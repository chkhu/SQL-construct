...
public:
     ...
     * @return `true` if a delete operation was performed, `false` if there are no delete performed
     *
...
private:
    ...
    bool is_init_ = false;
    unsigned long delete_size = 0;
    TableInfo *table_info_{};
    vector<IndexInfo *> table_indexes_{};
    vector<Row> delete_rows_{};

    bool GetDeleteTuples();

    [[maybe_unused]] bool CheckTupleExist(const Row &row);
